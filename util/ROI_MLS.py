...
def mls_affine_deformation_inv(
    image, height, width, channel, p, q, alpha=1.0, density=1.0
):
    ...
    w = (
        1.0 / np.sum((reshaped_p - reshaped_v) ** 2, axis=1) ** alpha
    )  # [ctrls, grow, gcol]
    w[w == np.inf] = 2 ** 31 - 1
    ...
    pTwq = np.sum(
        reshaped_phat * reshaped_w * reshaped_qhat, axis=0
    )  # [2, 2, grow, gcol]
    try:
    ...
    mul_right = np.sum(
        reshaped_phat * reshaped_w * reshaped_phat2, axis=0
    )  # [2, 2, grow, gcol]
    reshaped_mul_right = mul_right.transpose(2, 3, 0, 1)  # [grow, gcol, 2, 2]
    temp = np.matmul(
        np.matmul(reshaped_mul_left, inv_pTwq), reshaped_mul_right
    )  # [grow, gcol, 1, 2]
...
def mls_affine_deformation_inv_final(
    height, width, channel, p, q, alpha=1.0, density=1.0
):
    """Affine inverse deformation
    ### Params:
        * image - ndarray: original image
...
def mls_similarity_deformation_inv(
    image, height, width, channel, p, q, alpha=1.0, density=1.0
):
    ...
    mul_right = np.concatenate(
        (reshaped_phat1, reshaped_neg_phat_verti), axis=1
    )  # [ctrls, 2, 2, grow, gcol]
    mul_left = reshaped_qhat * reshaped_w  # [ctrls, 1, 2, grow, gcol]
    Delta = np.sum(
        np.matmul(
            mul_left.transpose(0, 3, 4, 1, 2), mul_right.transpose(0, 3, 4, 1, 2)
        ),
        axis=0,
    ).transpose(
        0, 1, 3, 2
    ...
    # Get final image transfomer -- 3-D array
    reshaped_v_minus_qstar_mul_mu = v_minus_qstar_mul_mu.reshape(
        1, 2, grow, gcol
    )  # [1, 2, grow, gcol]
    transformers = (
        np.matmul(reshaped_v_minus_qstar_mul_mu.transpose(2, 3, 0, 1), inv_B)
        .reshape(grow, gcol, 2)
        .transpose(2, 0, 1)
        + pstar
    )  # [2, grow, gcol]
...
def mls_rigid_deformation_inv(
    ...
    # Get final image transfomer -- 3-D array
    temp = (
        np.matmul(reshaped_vqstar.transpose(2, 3, 0, 1), inv_B)
        .reshape(grow, gcol, 2)
        .transpose(2, 0, 1)
    )  # [2, grow, gcol]
...
# mls rigid algorithm
def mls_rigid_deformation_inv_wy(
    image, height, width, channel, p, q, alpha=1.0, density=1.0
):
    """
     Rigid inverse deformation
    ...
    w = (
        1.0 / torch.sum((reshaped_p - reshaped_v) ** 2, dim=1) ** alpha
    )  # [ctrls, grow, gcol]
    w[w == torch.tensor(float("inf"))] = 2 ** 31 - 1
    pstar = torch.sum(w * reshaped_p.permute(1, 0, 2, 3), dim=1) / torch.sum(
        w, dim=0
    )  # [2, grow, gcol]
    phat = reshaped_p - pstar  # [ctrls, 2, grow, gcol]
    qstar = torch.sum(w * reshaped_q.permute(1, 0, 2, 3), dim=1) / torch.sum(
        w, dim=0
    )  # [2, grow, gcol]
    qhat = reshaped_q - qstar  # [ctrls, 2, grow, gcol]
    reshaped_phat1 = phat.reshape(ctrls, 1, 2, grow, gcol)  # [ctrls, 1, 2, grow, gcol]
    ...
    mul_right = torch.cat(
        (reshaped_phat1, reshaped_neg_phat_verti), dim=1
    )  # [ctrls, 2, 2, grow, gcol]
    mul_left = reshaped_qhat * reshaped_w  # [ctrls, 1, 2, grow, gcol]
    Delta = torch.sum(
        torch.matmul(mul_left.permute(0, 3, 4, 1, 2), mul_right.permute(0, 3, 4, 1, 2)),
        dim=0,
    ).permute(
        0, 1, 3, 2
    )  # [grow, gcol, 2, 1]
    ...
    # Get final image transfomer -- 3-D array
    temp = (
        torch.matmul(reshaped_vqstar.permute(2, 3, 0, 1), inv_B)
        .reshape(grow, gcol, 2)
        .permute(2, 0, 1)
    )  # [2, grow, gcol]
    ...
    transformed_image = image[
        tuple(transformers.numpy().astype(np.int16))
    ]  # [grow, gcol]
...
def roi_mls_whole(feature, d_point, g_point, step=1):
    ...
    grid, timg = mls_affine_deformation_inv(
        featureTmp.cpu(),
        height,
        width,
        channel,
...
def roi_mls_whole_final(feature, d_point, g_point, step=1):
    ...
    # featureTmp = feature.permute(1,2,0)
    # grid, timg = mls_rigid_deformation_inv_wy(featureTmp.cpu(), height, width, channel, g_land.cpu(), d_land.cpu(), density=1.) # 2 * 256 * 256 # wenyu
    grid = mls_affine_deformation_inv_final(
        height, width, channel, g_land.cpu().numpy(), d_land.cpu().numpy(), density=1.0
    )  # affine prefered
    # grid, timg_sim = mls_similarity_deformation_inv(featureTmp.cpu(), height, width, channel, g_land.cpu().numpy(), d_land.cpu().numpy(), density=1.) # similarity
    # grid, timg_rigid = mls_rigid_deformation_inv(featureTmp.cpu(), height, width, channel, g_land.cpu().numpy(), d_land.cpu().numpy(), density=1.) #rigid
